...
class Edit_Window(Toplevel):
    ...
    def __init__(self, root, main_root):
        ...
        self.b_add = Button(f3, text="INSERT", bd=5, width=7, bg='#e7d95a', font=('arial', 10, "bold"),
                            command=self.add)
        self.b_add.grid(row=0, column=0, padx=5)
        self.b_print = Button(f3, text="UPDATE BILL", bd=5, width=13, bg='#e7d95a', font=('arial', 10, "bold"),
                              command=self.gen)
        self.b_print.grid(row=0, column=1, padx=5)
        self.b_delete = Button(f3, text="DELETE BILL", bd=5, width=10, bg='#e7d95a', font=('arial', 10, "bold"),
                               command=self.delt)
        self.b_delete.grid(row=0, column=2, padx=5)
        self.b_reset = Button(f3, text="BACK", bd=5, width=5, bg='#e7d95a', font=('arial', 10, "bold"),
                              command=self.exit)
                    ...
                    try:
                        ...
                        if int(self.update_qnt) < 0:
                            messagebox.showwarning("Billing Software",
                                                   "In-sufficient Stock of '{}'".format(self.cb1.get()), parent=self)
                            self.q.set("")
                            self.qnt.focus_set()
                        else:
    ...
    def delt(self, event=""):
        try:
            if self.find_counter == 1:
                raise AttributeError
            m = messagebox.askyesno("Billing Software",
                                    "Are you want to Delete Bill of '{}' ?".format(self.c_name.get()), parent=self)
            if m > 0:
                ...
                try:
                    ...
                    messagebox.showinfo("Billing Software",
                                        "Bill of '{}' is Deleted Successfully !".format(self.c_name.get()), parent=self)
                    self.reset()
                except:
    ...
    def pre_designed(self, pdf):
        ...
        pdf.drawString(30, 705, "Updated Date: " + date)
    ...
    def gen(self, event=""):
        try:
            if self.find_counter == 1:
                raise AttributeError
            m = messagebox.askyesnocancel("Billing Software",
                                          "Are you want to Update Changes in the Bill of '{}' ?".format(
                                              self.c_name.get()), parent=self)
            if m == True:
                ...
                try:
                    ...
                    self.find_counter = 1
                    messagebox.showinfo("Billing Software",
                                        "Bill of {} is Saved Successfully in your Database !".format(self.c_name.get()),
                                        parent=self)
                    x = messagebox.askyesno("Billing Software",
                                            "Are you want to save Bill as PDF ?", parent=self)
                    self.cancel_counter = False
                    if x > 0:
                        self.pdfgenerate()
            ...
            messagebox.showinfo("Billing Software", "Please Find Bill First. After That You Can Update it! ",
                                parent=self)
            self.bid.focus_set()
    ...
    def watch(self, event=""):
        ...
        try:
            ...
            try:
                ...
                self.l_ttl.set(self.fetch_data[0][2])
                self.c_name.config(state="disabled")
                self.l_total.config(state="disabled")
