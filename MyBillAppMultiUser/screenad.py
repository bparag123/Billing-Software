...
class screenad1(Toplevel):
    ...
    def logadmin(self,event=""):
        ...
        query1 = """  select superuser from Users where username=?"""
        self.s_user=self.conn.execute(query1,(self.usernameentry.get(),)).fetchone()
        ...
        if( self.p_word[0] == self.passwordentry.get()):
            if(self.s_user[0] > 0):
                ...
                self.pword.set("")
                self.usernameentry.focus_set()
                obj_of_product = addproduct(self,self.main_root)
            else:
                messagebox.showerror("Billing Software","You are not assigned as Admin !",parent=self)
                return

        else:
            messagebox.showerror("Billing Software","Please Enter valid Password!",parent=self)
            self.pword.set("")
            self.passwordentry.focus_set()
            return
    ...
    def c_p(self,event=""):
        ...
        self.cpbutton.place(x=305, y=400)
    ...
    def change(self,event=""):
        ...
        try:
            ...
            m = messagebox.showinfo("Billing Software", "Please Re-enter Password.", parent=self)
        ...
        self.conn.commit()
        messagebox.showinfo("Billing Software", "Password of '{0}' is Changed Successfully.".format(self.cusernameentry.get()), parent=self)
        self.clr()
        self.des()
...
##=================================================login screen=============================================================
        ...
        loginframe= Frame(self,background=self.bgclr1, relief=SOLID,bd=1)
        loginframe.place(x=0,y=50,height=650,width=1350)

        llframe = LabelFrame(loginframe, background=self.bgclr1, text="Admin Login Info.", font=("Arial Bold", 20), relief=SOLID)
        ...
        loginbutton = Button(llframe, text="LOGIN", bg=self.bgclr2, command=self.logadmin, bd=5, width=15,font=("Arial Bold", 10))
        ...
        self.bind("<Return>", self.logadmin)
